...
class ActorRolloutRefWorker(Worker):
    ...
    def init_model(self):
        ...
        if self._is_ref:
            ...
            local_path = copy_to_local(ref_model_path)
        ...
        with self.rollout_sharding_manager:
            ...
            if self.config.rollout.name == "sglang_async":
                ...
                if "n" in prompts.meta_info:
                    output = self.rollout.generate_sequences(prompts=prompts, n=prompts.meta_info["n"])
                else:
                    output = self.rollout.generate_sequences(prompts=prompts)
    ...
    def compute_log_prob(self, data: DataProto):
        ...
        data.meta_info["save_top_k"] = self.config.rollout.save_top_k
        ...
        with self.ulysses_sharding_manager:
            ...
            if data.meta_info["save_top_k"] > 0:
                output, entropys, top_k_log_probs, top_k_input_ids = self.actor.compute_log_prob(data=data, calculate_entropy=True)
            else:
                output, entropys = self.actor.compute_log_prob(data=data, calculate_entropy=True)
            if data.meta_info["save_top_k"] > 0:
                output = DataProto.from_dict(
                    tensors={"old_log_probs": output, "entropys": entropys, "top_k_log_probs": top_k_log_probs, "top_k_input_ids": top_k_input_ids},
    ...
    @register(dispatch_mode=Dispatch.DP_COMPUTE_PROTO)
    def compute_minp_log_prob(self, data: DataProto):
        ...
        # Support all hardwares
        data = data.to(torch.cuda.current_device())
        # we should always recompute minp log_probs when it is HybridEngine
        data.meta_info["micro_batch_size"] = self.config.rollout.log_prob_micro_batch_size_per_gpu
        data.meta_info["max_token_len"] = self.config.rollout.log_prob_max_token_len_per_gpu
        data.meta_info["use_dynamic_bsz"] = self.config.rollout.log_prob_use_dynamic_bsz
        data.meta_info["temperature"] = self.config.rollout.temperature
        data.meta_info["minp"] = self.config.actor.minp_p_threshold
        data.meta_info["minp_soft"] = self.config.actor.minp_soft
        data.meta_info["regular_type"] = self.config.actor.regular_type
        # perform recompute minp log_prob
        with self.ulysses_sharding_manager:
            data = self.ulysses_sharding_manager.preprocess_data(data)
            output, entropys = self.actor.compute_log_prob(data=data, calculate_entropy=True, minp=data.meta_info["minp"], minp_soft=data.meta_info["minp_soft"], regular_type=data.meta_info["regular_type"])
            output = DataProto.from_dict(
                tensors={"minp_old_log_probs": output}, # , "entropys": entropys
                meta_info={"temperature": self.config.rollout.temperature},
            )
            output = self.ulysses_sharding_manager.postprocess_data(output)
        ...
        if self._is_offload_param:
            offload_fsdp_model_to_cpu(self.actor_module_fsdp)
            log_gpu_memory_usage("After offload actor model during compute_minp_log_prob", logger=logger)
