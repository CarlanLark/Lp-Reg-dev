...
def extract_answer_v2(model_response: str) -> str:
...
def compute_score(model_response, gt_answer, fast=False):
    ...
    try:
        ...
        # Handle timeout from math_verify operations
        is_correct = False
    except Exception:
        # Handle any other exceptions during grading
        is_correct = False
...
def compute_score_v2(model_response, gt_answer, fast=False, timeout_score=0.0):
    model_answer = extract_answer(model_response)
    if model_answer is None:
        return {
            "score": 0.,
    ...
    ground_truth_boxed = "\\boxed{" + gt_answer + "}"
    try:
        ret_score, _ = verify_func([ground_truth_boxed], [model_response])
    except Exception:
        pass
    except TimeoutException:
